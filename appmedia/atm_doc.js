	...
	*/
    ...
    function CardNameChanged(id, card_number) {
    	...
    	if (text.charAt(0) == '%') {
    		...
    		var d = new Date();
            d.setDate(d.getDate());
            document.getElementById("arrival_date"+card_number).value = d.getISODate();
    	}
    }
    ...
    function AddRowsToTable(count) {
		...
		for (var i = 0; i < count; i++) {
			...
			d.innerHTML = '<input id="arrival_date'+ CardsCount +'" name="arrival_date'+ CardsCount +'" type="text" class="DateField" />';
			e.innerHTML = '<input id="delivery_date'+ CardsCount +'" name="delivery_date'+ CardsCount +'" type="text" class="DateField" />';
